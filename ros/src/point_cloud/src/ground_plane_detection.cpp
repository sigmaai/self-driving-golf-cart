...
void cloud_callback_two (const sensor_msgs::PointCloud2ConstPtr& input){
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_pre(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PCLPointCloud2 pcl_pc2;
    pcl_conversions::toPCL(*input, pcl_pc2);
    pcl::fromPCLPointCloud2(pcl_pc2,*cloud_pre);

    // declare variables -----------------------------------------------------------------------------------------------
    pcl::GroundPlaneComparator<pcl::PointXYZ, pcl::Normal>::Ptr road_comparator;
    ...
    for (const auto &region_index : region_indices)
    {
        if (region_index.indices.size () > 1000)
        {
            ...
            {
                pcl::PointXYZ ground_pt (cloud_pre->points[region_index.indices[j]].x,
                                         cloud_pre->points[region_index.indices[j]].y,
                                         cloud_pre->points[region_index.indices[j]].z);
...
/*                ground_image->points[region_index.indices[j]].g = static_cast<pcl::uint8_t> ((cloud->points[region_index.indices[j]].g + 255) / 2);
            ...
            // Compute plane info
            pcl::computeMeanAndCovarianceMatrix (*cloud_pre, region_index.indices, clust_cov, clust_centroid);
...
void cloud_callback (const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    // ground plane detection stuff ------------------------------------------------------------------------------------
    pcl::GroundPlaneComparator<pcl::PointXYZ, pcl::Normal>::Ptr road_comparator;

    // Set up the groundplane comparator
    road_comparator->setInputCloud (temp_cloud);
    road_comparator->setInputNormals (cloud_normals);
    ...
    pcl::OrganizedConnectedComponentSegmentation<pcl::PointXYZ, pcl::Label> ground_segmentation(road_comparator);

    pcl::PointCloud<pcl::Label> labels;
    std::vector<pcl::PointIndices> region_indices;
    ground_segmentation.setInputCloud (temp_cloud);
    ground_segmentation.segment (labels, region_indices);
