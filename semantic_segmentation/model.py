...
from keras.layers import Input, merge, Conv2D, MaxPooling2D, AveragePooling2D, Lambda, Add
...
def segnet(nb_classes, optimizer=Adam(lr=0.001, epsilon=1e-08, decay=0.0), input_height=480, input_width=640):
    ...
    model.add(Conv2D(nb_classes, (1, 1), activation='sigmoid'))
...
def fcn_model():
    ...
    inputs = Input((configs.img_height, configs.img_width, 3))
...
def build_icnet(width=configs.img_width, height=configs.img_height, weights_path=None, train=False):
    ...
    out = Conv2D(3, 1, activation='softmax', name='conv6_cls')(y)

    if train:
        aux_1 = Conv2D(3, 1, activation='softmax', name='sub4_out')(aux_1)
        aux_2 = Conv2D(3, 1, activation='softmax', name='sub24_out')(aux_2)
