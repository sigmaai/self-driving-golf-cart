...
def train_nn(sess, training_epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss,
    ...
    for e in range(0, training_epochs):
        ...
        print('Epoch: {:02d}  -  Loss: {:.03f}'.format(e, loss_this_epoch / configs.batch_per_epoch))


def perform_tests():

    tests.test_for_kitti_dataset(configs.data_path)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Path to vgg model
        vgg_path = join(configs.data_path, 'vgg')

        # Create function to get batches
        batch_generator = gen_batch_function(join(configs.data_path, 'data_road/training'), (image_h, image_w))
        ...
        # Training parameters
        train_nn(sess, 30, 8, batch_generator, train_op, cross_entropy_loss,
                 image_input, labels, keep_prob, learning_rate)

        save_inference_samples("./", configs.data_path, sess, (image_h, image_w), logits, keep_prob, image_input)
