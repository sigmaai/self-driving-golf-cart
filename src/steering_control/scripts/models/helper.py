...
def get_initial_state(complex_state_tuple_sizes):
    ...
    init_state_flat = [tf.tile(multiples=[configs.BATCH_SIZE, 1], input=tf.get_variable("controller_initial_state_%d" % i, initializer=tf.zeros_initializer, shape=([1, s]), dtype=tf.float32))
